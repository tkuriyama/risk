...
-- All rendering starts from (rX, rY) coordinates in renderMain
...
showRect xStart yStart w h r c o =
    rect [ x (px xStart)
         ...
         , stroke <| Paint Color.black --Color.darkGrey
         ] []

-- Left == Undecided or Win; Right == Loss

showLeaf : Float -> Float -> Float -> Float -> Float -> Bool -> Int ->
           List (Svg msg)
showLeaf pNotLose xStart xEnd yStart yInc endLeaf d =
    let (_, xLenL, xLenR) = splitX xStart xEnd pNotLose
        leftFill =
            if endLeaf then Color.lightGreen else Color.lightGrey
        left =
            showRect xStart yStart xLenL (yInc * 0.5) 4 leftFill 0.5
        right =
            showRect (xStart+xLenL) yStart xLenR (yInc * 0.5) 4 (Color.lightRed) 0.5
        leftCarry =
            carryLeaves xStart xLenL yStart yInc Color.lightGreen (d-1)
        rightCarry =
            carryLeaves (xStart+xLenL) xLenR yStart yInc Color.lightRed (d-1)
    in [left, right] ++ (if endLeaf then leftCarry ++ rightCarry else [])

-- all losses must be carried down the tree
carryLeaves : Float -> Float -> Float -> Float -> Color.Color -> Int ->
              List (Svg msg)
...
getLeafXs start w ts =
    let f inc (s, acc) = (s+inc, ((s, s+inc) :: acc))
    in extractPs ts
       |> List.map (\p -> p * w)
       |> List.foldl f (start, [])
       |> \(_, acc) -> acc
...
showBranch xStart xEnd yStart yInc (Node p ts) d =
    let pNotLose =
            aggLevel p ts
        (xLen, xLenL, xLenR) =
            splitX xStart xEnd pNotLose
        xPairs =
            getLeafXs xStart xLen ts
        nextLevel t (xStart2, xEnd2) =
            showBranch xStart2 xEnd2 (yStart + yInc) yInc t (d-1)
    in (showLeaf pNotLose xStart xEnd yStart yInc (isLeaf ts) d) ++
       case isLeaf ts of
           True -> []
           _ -> (List.map2 nextLevel ts xPairs |> List.concat) ++
